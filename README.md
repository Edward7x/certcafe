...
1. 选择主菜单选项 `8`
2. 可开启或关闭全局 cron 续期任务
3. 可进入「指定域名管理自动续期」，查看 RSA/ECC 证书状态并单独启用或暂停某个主域名

全局 cron 是总开关：关闭后所有证书均不会自动续期；重新开启全局任务时，已单独暂停的域名仍保持暂停。域名级暂停采用 acme.sh 原生的 `.conf.removed` 机制，证书、私钥及安装文件均会保留，之后可从同一菜单恢复。
...
自动续期依赖 acme.sh 安装的一条全局 cron 任务，该任务统一检查全部未暂停的证书。签发流程默认开启，也可以在提示时选择关闭；之后可通过主菜单的「自动续期管理」控制全局任务，或单独启用/暂停指定主域名。若自动配置失败，可手动执行：
